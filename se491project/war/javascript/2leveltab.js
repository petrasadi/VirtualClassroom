...
function setTabsAsNotSelected() {
    var lis = document.getElementById("maintab").getElementsByTagName("li");
    for (var i = 0; i < lis.length; i++) {
        lis[i].setAttribute("class", "notselected");
    }
}
...
function initalizetab(tabid) {
    ...
    for (var i = 0; i < menuitems.length; i++) {
        if (menuitems[i].getAttribute("rel")) {
            menuitems[i].setAttribute("rev", tabid); // associate this submenu
            // with main tab
