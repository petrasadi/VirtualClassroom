...
public class GetDoesNotMeetExpectationsCount implements IDaoCommands {
    Key classesId;

    public GetDoesNotMeetExpectationsCount(Key classes)
    {
	...
	public Integer getExecute()
    {
        ...
        try {
           	for(Entity e : c.getClassRatings(classesId).asIterable()) {
           		...
           		for (String s : crList) {
           			if (s.equals("Does NOT Meet Expectations")) {
           				eeCount++;
           			}
           		}
           	}
